#include "robot_ethercat_adapters/ecmaster_hardware_bus.hpp"
...
EcMasterHardwareBus::~EcMasterHardwareBus()
{
  stop();
}

bool EcMasterHardwareBus::configure(
  const robot_hardware_api::HardwareParams & params,
  const std::vector<std::string> & joint_names,
  std::string & error)
{
  if (joint_names.empty()) {
    error = "EcMasterHardwareBus requires at least one joint";
    return false;
  }
  ...
  // Default Master policy is ObservationOnly; SI adapters must request motion.
  master_ = std::make_unique<ethercat_master_ecmaster::Master>(
    0, ethercat_master_ecmaster::MotionPolicy::SupervisedMotion);
  if (!master_->init(error)) {
    master_.reset();
    return false;
  }

  std::vector<ethercat_master_ecmaster::AxisConfig> axes;
  ...
  cmd_buf_.assign(joint_names_.size(), ethercat_master_ecmaster::AxisCommand{});
  state_buf_.assign(joint_names_.size(), ethercat_master_ecmaster::AxisState{});
  configured_ = true;
  error.clear();
  return true;
}

bool EcMasterHardwareBus::start(std::string & error)
{
  if (!configured_ || !master_) {
    error = "EcMasterHardwareBus not configured";
    return false;
  }
  if (!master_->start(error)) {
    return false;
  }
  // start() asserts safe-output; accept that safety event and arm healthy dwell so
  // exchange can release_safe_output once motion_reenable_allowed becomes true.
...
bool EcMasterHardwareBus::exchange(
...
{
  if (!running_ || !master_) {
    error = "EcMasterHardwareBus is not running";
    return false;
  }
  ...
  // start() asserts safe-output; after /request_safety_reset + healthy dwell, release
  // before setpoints can be injected. Only attempt when dwell already allows re-enable.
...
PLUGINLIB_EXPORT_CLASS(
  robot_ethercat_adapters::EcMasterHardwareBus,
